...
const NotasCredito = ({ searchTerm }) => {
  ...
  const fetchNotasCredito = async () => {
    try {
      const response = await axios.get(`${API}/notas-credito`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!notaData.numero_nota) {
        notaData.numero_nota = `NC-${Date.now()}`;
      }

      await axios.post(`${API}/notas-credito`, notaData);
      ...
      console.error("Error saving nota credito:", error);
    }
  };

  const aplicarNota = async (notaId) => {
    try {
      await axios.put(`${API}/notas-credito/${notaId}/aplicar`);
      fetchNotasCredito();
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-32">
        <div className="text-lg">Cargando notas de crédito...</div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="factura_id">Factura Relacionada *</Label>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <Label>Items de la Nota de Crédito *</Label>
              ...
              <div className="space-y-2 mt-4 p-3 bg-red-50 rounded">
                <div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-bold text-red-600">Total: ${total.toFixed(2)}</p>
                </div>
              </div>
            </div>
      ...
      {filteredNotas.length === 0 && (
        <div className="text-center py-8">
          <p className="text-gray-500">
            {searchTerm ? "No se encontraron notas de crédito con ese criterio" : "No hay notas de crédito registradas"}
          </p>
        </div>
      )}
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Detalle de la Nota de Crédito</DialogTitle>
          </DialogHeader>
          {viewingNota && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="mt-2 p-3 bg-red-50 rounded">
                  ...
                  <p className="font-bold text-red-600">Total: ${viewingNota.total}</p>
                </div>
              </div>
